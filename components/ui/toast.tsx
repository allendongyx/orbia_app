...
const toastVariants = cva(
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-3 overflow-hidden rounded-lg border p-4 pr-10 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full',
  {
    variants: {
      variant: {
        success:
          'success group border-emerald-700 bg-emerald-700 text-white',
        error:
          'error group border-rose-700 bg-rose-700 text-white',
      },
    },
    defaultVariants: {
      variant: 'success',
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      'inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium ring-offset-background transition-colors hover:bg-secondary focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 group-[.error]:border-white/30 group-[.error]:hover:bg-white/20 group-[.error]:text-white group-[.error]:focus:ring-white/50 group-[.success]:border-white/30 group-[.success]:hover:bg-white/20 group-[.success]:text-white group-[.success]:focus:ring-white/50',
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      'absolute right-2 top-2 rounded-md p-1 text-white/90 transition-all hover:text-white hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-white/50',
      className
    )}
    toast-close=""
    {...props}
